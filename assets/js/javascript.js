...
function runPage() {
  ...
  stationRef.on("child_added", function(snapshot) {
    ...
    cardDelBtn.on("click", function() {
      ...
      var ref = firebase
        .database()
        .ref("users/" + uid + "/stations/" + childKey);
      ref.remove();
    });
    ...
    refresh = new Refresh(
      ...
      setTOD,
      todDiv.attr("id"),
      ttlDiv.attr("id"), //Time of day.
      vane,
      vaneSpeed.attr("id"),
      vaneDir.attr("id"), //Wind vane.
      setClouds,
      clouds,
      cloudDiv.attr("id"), //Cloud cover.
      cldDiv.attr("id"),
      cltDiv.attr("id"),
      thisTemp,
      tempF.attr("id"),
      tempC.attr("id"), //Temperature.
      tempType,
      tempDiv.attr("id"),
      temp,
      setHumidity,
      humDiv.attr("id"), //Humidity.
      setPressure,
      presDiv.attr("id"), //Pressure.
      currentDiv.attr("id"),
      curDiv.attr("id"), //Current conditions.
  ...
  class Refresh {
    ...
    initListener() {
      ...
      tDiv.addEventListener("click", function() {
        self.tempType === self.F
          ? (self.tempType = self.C)
          : (self.tempType = self.F);
    ...
    doRefresh() {
      ...
      if (moment().diff(this.localTS) > 10 * 60 * 1000 + 10000) {
        ...
        //Build the query URL string.
        var queryString = this.isZip
          ? "zip=" + this.zipString + ",us"
        ...
        $.ajax({
          ...
          success: function(response) {
            ...
            document.getElementById(self.tempF).innerHTML =
              "<p>Fahrenheit: " +
              self.tempCalc(temp, self.F).toFixed(1) +
              "&#8457</p>";
            document.getElementById(self.tempC).innerHTML =
              "<p>Celsius: " +
              self.tempCalc(temp, self.C).toFixed(1) +
              "&#8451</p>";
            ...
            document.getElementById(self.curDiv).innerHTML =
              "<p>" + curDesc + "</p>";
